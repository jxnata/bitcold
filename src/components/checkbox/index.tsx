...
const useCheckbox = (options: string[], initialSelected?: string[], unique?: boolean) => {
	...
	const { t } = useTranslation('translation', { keyPrefix: 'checkbox' })
			...
			return (
				<S.Container>
					{options.map(option => (
						...
						>
							<S.ButtonText aria-selected={isSelected(option)}>{t(option)}</S.ButtonText>
						</S.Button>
					))}
				</S.Container>
			)
		},
		[options, isSelected, t, onChangeSelected]
	)
